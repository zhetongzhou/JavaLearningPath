...
public class DB {
	...
	public DB(){
		try{
			...
			ct=DriverManager.getConnection("jdbc:mysql://localhost:3306/test","root","123456");
			}catch(Exception e){
				e.printStackTrace();
			}
	}
	...
	public boolean addInfo(LyTable ly){
		try{
			...
			pstmt.setInt(1, ly.getUserId());/////////////////////ly.getUserId()
			pstmt.setDate(2, ly.getDate());
			pstmt.setString(3, ly.getTitle());//ly.getTitle()
			pstmt.setString(4, ly.getContent());//ly.getContent()
			pstmt.executeUpdate();
			return true;
		}catch(Exception e){
			e.printStackTrace();
